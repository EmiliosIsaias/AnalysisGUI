...
%RAM CHANGED TO 0 to GET RID OF PROGRESSBAR October 2014
...
for i=1:length(varargin)
    if ischar(varargin{i})==1
        ...
        switch varargin{i}
            case 'progress'
                if Info.blocks>10
                    ShowProgress=0; %RAM CHANGED TO 0 to GET RID OF PROGRESSBAR October 2014
                  %RAM  progbar=progressbar(0,sprintf('Analyzing %d blocks on channel %d',Info.blocks,chan),...
                     %RAM    'Name',sprintf('%s',fopen(fid)));
                end;
            case 'scale'
                ScaleData=1;
            case 'mat'
...
if NumFrames==1
    ...
    for i=startEpoch:endEpoch
        ...
        if ShowProgress==1 && rem(i,10)==0
          ...
          %RAM   progressbar(done, progbar,...
            %RAM     sprintf('Reading Channel %d....     %3.0f%% Done',chan,done*100));
        end;
    end;
...
if ScaleData==1 && MatFlag==0
    if ShowProgress==1
    %RAM     progressbar(1,progbar,'Scaling data.....');
    end;
    s=h.scale/6553.6;
    o=h.offset;
    data=data*s+o;
