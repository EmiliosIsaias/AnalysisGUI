...
% October 2014, R. Mease:  removed waitbar to get rid of errors in Matlab
% 2014b
%
...
% RAM progbar=progressbar(0,'','Name', fopen(fid));
N=length(c);
% Import the data.

wait_handle=waitbar(0,'Importing SMR data...');
for i=1:N
    waitbar(i/N,wait_handle)
    chan=c(i).number;
      % RAM  progressbar(chan/N, progbar, ...
       % RAM  sprintf('Importing data on Channel %d',chan));
    try
    ...
    if ~isempty(data)
      % RAM  progressbar(i/N, progbar,sprintf('Saving Channel %d',chan));
